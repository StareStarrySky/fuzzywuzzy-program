...
def calculate_spec_similarity(row_name, spec_csv):
    print('=======')
    print(row_name.name)
    return process.extractBests(row_name['SPEC_MODEL_NUMBER'],
                                spec_csv.loc[row_name.name:(spec_csv.shape[0] - 1), 'SPEC_MODEL_NUMBER'],
                                limit=100, score_cutoff=95)
    ...
    for _, row in csv_group.iterrows():
        ...
        if len(simi) > 0 and simi[0][2] not in moved_row:
            ...
            if conceited and temp_size > 1:
                if 'spec_similarity' not in csv_temp.columns:
...
wl['wl_name_similarity'] = wl.swifter.apply(lambda name_row: calculate_wl_name_similarity(name_row, wl), axis=1)

print(time.strftime("%Y-%m-%d %H:%M:%S -- ", time.localtime()) + '打分完成')

wl_new = group_by(wl, 'MATERIEL_NAME', 'wl_name_similarity')

print(time.strftime("%Y-%m-%d %H:%M:%S -- ", time.localtime()) + '筛选完成')
